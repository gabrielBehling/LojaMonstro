...
function updateCarrossel() {
    ...
    items.forEach((item, index) => {
        item.style.display = (index >= currentSlideIndex && index < currentSlideIndex + itemsPerPage) ? 'flex' : 'none';
    });
}

// Função para mudar o slide
function mudarSlide(direction) {
    const totalItems = document.querySelectorAll('#produtos .produto-item').length;
